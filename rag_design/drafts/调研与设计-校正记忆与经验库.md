...
**日期：** 2026-03-28  
**状态：** 草案 · 可迭代（工作副本；成稿同步见 `rag_design/docs/plans/`）

**依据：** [需求说明-校正记忆与经验库.md](../docs/需求说明-校正记忆与经验库.md) · [基于 EvoPalantir 的现实对齐模拟方案.md](../../knowledge/基于%20EvoPalantir%20的现实对齐模拟方案.md) §4.8–5.1 · [自进化Agent…](../docs/自进化Agent：经验写回的运行时记忆闭环机制/自进化Agent：经验写回的运行时记忆闭环机制.md)
...
综述与对比表见 [自进化Agent…](../docs/自进化Agent：经验写回的运行时记忆闭环机制/自进化Agent：经验写回的运行时记忆闭环机制.md)。下表仅 **映射到校正记忆**（队友可继续补论文级引用）。
