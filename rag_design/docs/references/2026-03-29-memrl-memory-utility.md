...
| 维度 | 外部方案 | 本仓库（出处） |
|------|----------|----------------|
| 记忆形态 | 情景记忆 + RL 信号驱动的选择与更新（论文表述） | `CaseRecord` + 可选 `utility_score`（[宪章](../plans/2026-03-28-调研与设计-校正记忆与经验库.md) §2.1、§1.4 Retrieve/Governance） |
| 检索 | 两阶段检索、强调效用/反馈（论文） | **两阶段检索（粗召回 + 精排/效用）**（宪章 §1.4 RetrieveModule） |
