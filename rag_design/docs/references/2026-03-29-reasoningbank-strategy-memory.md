...
| 维度 | 外部方案 | 本仓库（出处） |
|------|----------|----------------|
| 记忆内容 | 推理策略记忆（论文）；代码侧分 Web/SWE 目录 | `CaseRecord` + 可选 `reflection_text`；**RuleBook** 短规则（宪章 §2.1、§1.4 Governance） |
| 闭环 | 测试时检索 + 任务后写回 | 校正完成后 **Ingest → CaseStore → Index**；**Governance 异步**（宪章 §1.4） |
