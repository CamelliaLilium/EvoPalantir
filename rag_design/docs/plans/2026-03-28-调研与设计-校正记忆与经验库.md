...
**日期：** 2026-03-28  
**状态：** 成稿 v1.0 · 可迭代（草稿：`rag_design/drafts/调研与设计-校正记忆与经验库.md`）

**权威来源：** [knowledge/基于 EvoPalantir 的现实对齐模拟方案.md](../../../knowledge/基于%20EvoPalantir%20的现实对齐模拟方案.md) §4.8–4.12、§5.1 · [需求说明-校正记忆与经验库.md](../需求说明-校正记忆与经验库.md) · [agentself-evolution.md](../agentself-evolution.md) · [自进化Agent…](../自进化Agent：经验写回的运行时记忆闭环机制/自进化Agent：经验写回的运行时记忆闭环机制.md)
...
**留痕与案例语义分离。** **Run 级留痕**（实验跟踪、回放索引）与 **案例级语义存储与检索** **不得混为同一概念**。V1 默认：**Run 留痕** 走 **MLflow Tracking**；**案例检索** 走 **CaseStore + Index**（结构化必选，向量索引可选）。依据见 [agentself-evolution.md](../agentself-evolution.md) §1 与 §6。
...
编排 **单次** 「校正完成」处理流程的 **无状态（或仅幂等状态）** 组件。对标 AOS Charter 中 **薄协调器**：知顺序，不知各模块内部实现。
...
**V1 默认：MLflow Tracking**（知识库 §5.1、[agentself-evolution.md](../agentself-evolution.md) §1）。通过 **TraceSink 接口** 封装，以便替换为 W&B、Neptune、自建表等。
...
- **Mesa DataCollector 思想**（[agentself-evolution.md](../agentself-evolution.md) §2）：字段分层（model-level / agent-level / tables）**启发** CaseRecord 的 **指标块** 组织；**不强制** 依赖 Mesa 库。  
...
`CaseRecord` 的 **唯一正式持久化边界**（对标 AOS 中 Store 接口）。
